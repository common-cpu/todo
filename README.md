...
環境変数 `ASANA_PROJECTS` で設定したプロジェクトが対象です。デフォルト例：
...
| 🔴 期限切れ | 期限が過去の未完了タスク |
| 🟡 本日期限 | 今日が期限のタスク |
| 🟠 ３日以内期限 | 明日〜3日後が期限のタスク |
| 🔵 今週期限 | 今週（月〜金）が期限のタスク（上記に含まれないもの） |
| ⚪ 期限未設定 | 期限が設定されていない未完了タスク |
...
| `ASANA_PROJECTS` | 対象プロジェクトの `{name, gid}` 配列のJSON |
...
```json
[
  {"name":"Provia","gid":"1234567890"},
  {"name":"DataArt","gid":"2345678901"},
  {"name":"BlueVoyant","gid":"3456789012"},
  {"name":"Blueship","gid":"4567890123"},
  {"name":"ZIPAIR Tokyo","gid":"5678901234"}
]
```
...
- 毎日 JST 9:00 に自動実行（cron: `0 0 * * *` UTC）
- 手動実行も可能
